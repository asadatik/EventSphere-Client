...
const OrganizerRequestButtons = ({ user, dialogType, handleCancel, handleAccept }) => {
    ...
    const mutation = useMutation({
        mutationFn: async (id) => {
            const res = await axios.put(`https://event-sphare-server-one.vercel.app/userRollUpdated/${id}`);
            return res.data;
        },
        onSuccess: (data) => {
            if (data.success) {
    ...
    const mutation2 = useMutation({
        mutationFn: async (id) => {
            const res = await axios.put(`https://event-sphare-server-one.vercel.app/organizingRequestCancel/${id}`);
            return res.data;
        },
        onSuccess: (data) => {
            if (data.success) {
