...
const OrganizerRequestButtons = ({ user, dialogType, handleCancel, handleAccept }) => {
    ...
    const mutation = useMutation({
        mutationFn: async (id) => {
            const res = await axios.put(`https://eventsphare-server-psun.onrender.com/userRollUpdated/${id}`);
            return res.data;
        },
        onSuccess: (data) => {
            if (data.success) {
    ...
    const mutation2 = useMutation({
        mutationFn: async (id) => {
            const res = await axios.put(`https://eventsphare-server-psun.onrender.com/organizingRequestCancel/${id}`);
            return res.data;
        },
        onSuccess: (data) => {
            if (data.success) {
