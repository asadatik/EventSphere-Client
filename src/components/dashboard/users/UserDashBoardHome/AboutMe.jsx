...
const AboutMe = () => {
  ...
  const { data, isLoading, refetch } = useQuery({
      queryKey: ["user"],
      queryFn: () =>
          fetch(`https://eventsphare-server-psun.onrender.com/user/${lastPathSegment}`).then((res) =>
              res.json()
          ),
  });
  return (
    <div className="px-4 lg:px-8">
      <div>
        <h2 className="text-xl lg:text-2xl text-left text-[#1b85db] font-bold">About Me</h2>
      ...
      <div>
        <h2 className="text-xl lg:text-2xl text-left mt-6 text-[#1b85db] font-bold">Skills</h2>
        <div className="flex flex-wrap">
          {[data?.skills || 'PhotoShop', data?.specialty || 'ReactJS', 'BoostTrap', 'Responsive', 'Crypto'].map(skill => (
            <div className="p-2" key={skill}>
              <div className="p-1 px-2 rounded-md bg-[#1b85db] text-sm lg:text-base  text-white transition duration-300">
                {skill}
              </div>
            </div>
          ))}
      ...
      <div>
        <h2 className="text-xl lg:text-2xl text-left mt-6 mb-3 text-[#1b85db] font-bold">Language</h2>
      ...
      <div>
        <h2 className="text-xl lg:text-2xl mt-6 mb-3 text-left text-[#1b85db] font-bold">Personal Information</h2>
        <div>
