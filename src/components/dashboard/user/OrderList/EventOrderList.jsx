...
const EventOrderList = () => {
  ...
  const { data: invoice = [], refetch, isLoading } = useQuery({
    ...
    queryFn: () =>
      fetch(`https://eventsphare-server-psun.onrender.com/myAllOrder/${session?.data?.user?.email}`).then((res) =>
        res.json()
      ),
  });
  console.log(invoice);
  ...
  const handleRefundRequest = async (id) => {
    const res = await axios.put(`https://eventsphare-server-psun.onrender.com/refundRequest/${id}`)
    console.log(res)
